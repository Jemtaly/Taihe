...
struct same_impl_t<
  ...
  bool operator()(data_view lhs, data_view rhs) const {
    ...
    env_guard guard;
    ani_env *env = guard.get_env();
    ::taihe::expected<uintptr_t, ::taihe::error> lhs_ptr =
        lhs_as_ani->getGlobalReference();
    ::taihe::expected<uintptr_t, ::taihe::error> rhs_ptr =
        rhs_as_ani->getGlobalReference();
    if (!lhs_ptr || !rhs_ptr) {
      return false;
    }
...
namespace taihe {
inline __attribute__((noinline)) ani_module
...
inline __attribute__((noinline)) ani_namespace
...
inline __attribute__((noinline)) ani_class
...
inline __attribute__((noinline)) ani_enum
...
inline __attribute__((noinline)) ani_function ani_find_module_function(
    ani_env *env, ani_module mod, char const *name, char const *signature) {
  ani_function fn;
  if (mod == nullptr) {
    return nullptr;
  }
  if (ANI_OK != env->Module_FindFunction(mod, name, signature, &fn)) {
    std::cerr << "Function not found: " << " with signature: " << signature
              << std::endl;
    return nullptr;
  }
  return fn;
...
inline __attribute__((noinline)) ani_function ani_find_namespace_function(
    ani_env *env, ani_namespace ns, char const *name, char const *signature) {
  ani_function fn;
  if (ns == nullptr) {
    return nullptr;
  }
...
inline __attribute__((noinline)) ani_method ani_find_class_method(
    ani_env *env, ani_class cls, char const *name, char const *signature) {
  ani_method mtd;
  if (cls == nullptr) {
    return nullptr;
  }
  if (ANI_OK != env->Class_FindMethod(cls, name, signature, &mtd)) {
    std::cerr << "Method not found: " << " with signature: " << signature
              << std::endl;
    return nullptr;
  }
  return mtd;
...
inline __attribute__((noinline)) ani_static_method ani_find_class_static_method(
    ani_env *env, ani_class cls, char const *name, char const *signature) {
  ...
  if (ANI_OK != env->Class_FindStaticMethod(cls, name, signature, &mtd)) {
    std::cerr << "Static method not found: " << " with signature: " << signature
              << std::endl;
    return nullptr;
  }
  return mtd;
...
inline __attribute__((noinline)) ani_variable
...
inline __attribute__((noinline)) ani_variable
ani_find_namespace_variable(ani_env *env, ani_namespace ns, char const *name) {
  ani_variable var;
  if (ns == nullptr) {
    return nullptr;
  }
...
inline __attribute__((noinline)) ani_field
ani_find_class_field(ani_env *env, ani_class cls, char const *name) {
  ani_field fld;
  if (cls == nullptr) {
    return nullptr;
  }
...
inline __attribute__((noinline)) ani_static_field
ani_find_class_static_field(ani_env *env, ani_class cls, char const *name) {
  ani_static_field fld;
  if (cls == nullptr) {
    return nullptr;
  }
  if (ANI_OK != env->Class_FindStaticField(cls, name, &fld)) {
    std::cerr << "Static field not found: " << std::endl;
    return nullptr;
  }
  return fld;
}
...
inline ani_function ani_cache_module_function(ani_env *env) {
  static ani_function function =
      ani_find_module_function(env, ani_cache_module<descriptor_t>(env),
                               name_t.c_str(), signature_t.c_str());
  return function;
}
...
inline ani_function ani_cache_namespace_function(ani_env *env) {
  static ani_function function =
      ani_find_namespace_function(env, ani_cache_namespace<descriptor_t>(env),
                                  name_t.c_str(), signature_t.c_str());
  return function;
}
...
inline ani_method ani_cache_class_method(ani_env *env) {
  static ani_method method =
      ani_find_class_method(env, ani_cache_class<descriptor_t>(env),
                            name_t.c_str(), signature_t.c_str());
  return method;
}
...
inline ani_static_method ani_cache_class_static_method(ani_env *env) {
  static ani_static_method method =
      ani_find_class_static_method(env, ani_cache_class<descriptor_t>(env),
                                   name_t.c_str(), signature_t.c_str());
  return method;
}
...
inline ani_variable ani_cache_module_variable(ani_env *env) {
  static ani_variable variable = ani_find_module_variable(
      env, ani_cache_module<descriptor_t>(env), name_t.c_str());
  return variable;
}

template<nullable_fixed_string descriptor_t, nullable_fixed_string name_t>
inline ani_variable ani_cache_namespace_variable(ani_env *env) {
  static ani_variable variable = ani_find_namespace_variable(
      env, ani_cache_namespace<descriptor_t>(env), name_t.c_str());
  return variable;
}

template<nullable_fixed_string descriptor_t, nullable_fixed_string name_t>
inline ani_field ani_cache_class_field(ani_env *env) {
  static ani_field field = ani_find_class_field(
      env, ani_cache_class<descriptor_t>(env), name_t.c_str());
  return field;
}

template<nullable_fixed_string descriptor_t, nullable_fixed_string name_t>
inline ani_static_field ani_cache_class_static_field(ani_env *env) {
  static ani_static_field field = ani_find_class_static_field(
      env, ani_cache_class<descriptor_t>(env), name_t.c_str());
  return field;
}
}  // namespace taihe
...
#define TH_ANI_FIND_MODULE(env, descriptor) \
  ::taihe::ani_cache_module<descriptor>(env)
#define TH_ANI_FIND_NAMESPACE(env, descriptor) \
  ::taihe::ani_cache_namespace<descriptor>(env)
#define TH_ANI_FIND_CLASS(env, descriptor) \
  ::taihe::ani_cache_class<descriptor>(env)
#define TH_ANI_FIND_ENUM(env, descriptor) \
  ::taihe::ani_cache_enum<descriptor>(env)
...
#define TH_ANI_FIND_MODULE_VARIABLE(env, descriptor, name) \
  ::taihe::ani_cache_module_variable<descriptor, name>(env)
...
#define TH_ANI_FIND_CLASS_FIELD(env, descriptor, name) \
  ::taihe::ani_cache_class_field<descriptor, name>(env)
...
#define TH_ANI_FIND_MODULE(penv, descriptor)             \
  ([env = (penv)] {                                      \
    static ::taihe::sref_guard __guard(                  \
        env, ::taihe::ani_find_module(env, descriptor)); \
...
#define TH_ANI_FIND_NAMESPACE(penv, descriptor)             \
  ([env = (penv)] {                                         \
    static ::taihe::sref_guard __guard(                     \
        env, ::taihe::ani_find_namespace(env, descriptor)); \
...
#define TH_ANI_FIND_CLASS(penv, descriptor)             \
  ([env = (penv)] {                                     \
    static ::taihe::sref_guard __guard(                 \
        env, ::taihe::ani_find_class(env, descriptor)); \
...
#define TH_ANI_FIND_ENUM(penv, descriptor)             \
  ([env = (penv)] {                                    \
    static ::taihe::sref_guard __guard(                \
        env, ::taihe::ani_find_enum(env, descriptor)); \
...
#define TH_ANI_FIND_MODULE_FUNCTION(penv, descriptor, name, signature)     \
  ([env = (penv)] {                                                        \
    static ::taihe::sref_guard __guard(                                    \
        env, ::taihe::ani_find_module(env, descriptor));                   \
    static ani_function __function = ::taihe::ani_find_module_function(    \
        env, static_cast<ani_module>(__guard.get_ref()), name, signature); \
    return __function;                                                     \
  }())
...
#define TH_ANI_FIND_NAMESPACE_FUNCTION(penv, descriptor, name, signature)     \
  ([env = (penv)] {                                                           \
    static ::taihe::sref_guard __guard(                                       \
        env, ::taihe::ani_find_namespace(env, descriptor));                   \
    static ani_function __function = ::taihe::ani_find_namespace_function(    \
        env, static_cast<ani_namespace>(__guard.get_ref()), name, signature); \
    return __function;                                                        \
  }())
...
#define TH_ANI_FIND_CLASS_METHOD(penv, descriptor, name, signature)       \
  ([env = (penv)] {                                                       \
    static ::taihe::sref_guard __guard(                                   \
        env, ::taihe::ani_find_class(env, descriptor));                   \
    static ani_method __method = ::taihe::ani_find_class_method(          \
        env, static_cast<ani_class>(__guard.get_ref()), name, signature); \
    return __method;                                                      \
  }())
...
#define TH_ANI_FIND_CLASS_STATIC_METHOD(penv, descriptor, name, signature)     \
  ([env = (penv)] {                                                            \
    static ::taihe::sref_guard __guard(                                        \
        env, ::taihe::ani_find_class(env, descriptor));                        \
    static ani_static_method __method = ::taihe::ani_find_class_static_method( \
        env, static_cast<ani_class>(__guard.get_ref()), name, signature);      \
    return __method;                                                           \
  }())
...
#define TH_ANI_FIND_MODULE_VARIABLE(penv, descriptor, name)             \
  ([env = (penv)] {                                                     \
    static ::taihe::sref_guard __guard(                                 \
        env, ::taihe::ani_find_module(env, descriptor));                \
    static ani_variable __variable = ::taihe::ani_find_module_variable( \
        env, static_cast<ani_module>(__guard.get_ref()), name);         \
    return __variable;                                                  \
  }())
...
#define TH_ANI_FIND_NAMESPACE_VARIABLE(penv, descriptor, name)             \
  ([env = (penv)] {                                                        \
    static ::taihe::sref_guard __guard(                                    \
        env, ::taihe::ani_find_namespace(env, descriptor));                \
    static ani_variable __variable = ::taihe::ani_find_namespace_variable( \
        env, static_cast<ani_namespace>(__guard.get_ref()), name);         \
    return __variable;                                                     \
  }())
...
#define TH_ANI_FIND_CLASS_FIELD(penv, descriptor, name)        \
  ([env = (penv)] {                                            \
    static ::taihe::sref_guard __guard(                        \
        env, ::taihe::ani_find_class(env, descriptor));        \
    static ani_field __field = ::taihe::ani_find_class_field(  \
        env, static_cast<ani_class>(__guard.get_ref()), name); \
    return __field;                                            \
  }())
...
#define TH_ANI_FIND_CLASS_STATIC_FIELD(penv, descriptor, name)              \
  ([env = (penv)] {                                                         \
    static ::taihe::sref_guard __guard(                                     \
        env, ::taihe::ani_find_class(env, descriptor));                     \
    static ani_static_field __field = ::taihe::ani_find_class_static_field( \
        env, static_cast<ani_class>(__guard.get_ref()), name);              \
    return __field;                                                         \
  }())
