...
public:
  ...
  explicit error(::taihe::string_view message, int32_t code)
...
public:
  ...
  template<class E2>
  friend constexpr bool operator==(unexpected const &x,
...
public:
  ...
  template<class G>
  constexpr expected(unexpected<G> const &other)
      : has_val(false), unex(other.error()) {}

  template<class G>
  constexpr expected(unexpected<G> &&other)
      : has_val(false), unex(std::move(other.error())) {}
