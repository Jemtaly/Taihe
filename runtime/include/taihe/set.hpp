...
public:
  ...
  std::pair<item_t *, bool> emplace(as_param_t<K> key) const {
    ...
    while (*current_ptr) {
      if (is_same((*current_ptr)->item, key)) {
        if (cover) {
  ...
  struct iterator {
    ...
    iterator(node_t **bucket, node_t *current, std::size_t index,
             std::size_t cap)
...
private:
  ...
  friend bool taihe::is_same_adl(adl_tag_t, set_view lhs, set_view rhs);
...
template<typename K>
inline bool is_same_adl(adl_tag_t, set_view<K> lhs, set_view<K> rhs) {
