...
public:
  ...
  using vtable_type = int32_t(cb_out *_taihe_out, TCallback,
                              as_abi_t<Params>...);
...
public:
  ...
  ::taihe::expected<Return, ::taihe::error> operator()(
      Params... params) const & {
    return call_abi_func<cb_out, Return, callback_view, Params...>(
        m_handle.vtbl_ptr, *this, ::std::forward<Params>(params)...);
  }

public:
  template<typename Impl>
  static int32_t vtbl_impl(cb_out *_taihe_out, TCallback tobj,
                           as_abi_t<Params>... params) {
    return call_cpp_method<cb_out, Return, Params...>(
        _taihe_out, &Impl::operator(), *cast_data_ptr<Impl>(tobj.data_ptr),
        params...);
  };
