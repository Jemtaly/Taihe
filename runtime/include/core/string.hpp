...
struct string_view {
    ...
    bool empty() const noexcept { return tstr_len(m_handle) == 0; }

    size_type size() const noexcept { return tstr_len(m_handle); }
    ...
    const_pointer c_str() const noexcept { return tstr_buf(m_handle); }

    const_pointer data() const noexcept { return tstr_buf(m_handle); }

    const_iterator begin() const noexcept { return tstr_buf(m_handle); }
...
inline string to_string(T value) {
    ...
    // *result.ptr = '\0'; // std::to_chars does not write '\0' at the end of
    // the buffer automatcally
