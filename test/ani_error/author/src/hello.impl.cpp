...
::taihe::expected<void, ::taihe::error> callcb(::taihe::callback_view<void()> f) {
    ...
    if (!res.has_value()) {
        std::cout << "catch error in cpp callcb: " << res.error().message() << ", code: " << res.error().code_or(0)  << std::endl;
    }
    return res;
}
...
::taihe::expected<void, ::taihe::error> callcb_ii(::taihe::callback_view<int32_t(int32_t a)> f) {
    ...
    if (res.has_value()) {
        ...
        std::cout << "catch error in cpp callcb_ii: " << res.error().message() << ", code: " << res.error().code_or(0)  << std::endl;
    }
    return {};
}
}
