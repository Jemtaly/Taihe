...
public:
    ...
    string getTestStr() {
        std::cout << "NameSpaceImpl: " << __func__ << " " << testStr_
                  << std::endl;
        return testStr_;
    }
...
int8_t baseFunctionTest16(int8_t param1) {
    ...
    if (result > INT8_MAX || result < INT8_MIN) {
        taihe::core::set_error(
            "baseFunctionTest16: result exceeds int8_t range");
    }
    // 返回结果
    return static_cast<int8_t>(result);
}
...
int32_t baseFunctionTest42_int(int32_t param1, int16_t param2) {
    ...
    int64_t result =
        static_cast<int64_t>(param1) + static_cast<int64_t>(param2);
    std::cout << "NameSpaceImpl: " << __func__ << " result " << result
              << std::endl;
    if (result > INT32_MAX || result < INT32_MIN) {
        taihe::core::set_error(
