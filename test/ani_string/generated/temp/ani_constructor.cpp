#include "string_io.ani.hpp"
#include "string_op.ani.hpp"
#if __has_include(<ani.h>)
#include <ani.h>
#elif __has_include(<ani/ani.h>)
#include <ani/ani.h>
#else
#error "ani.h not found. Please ensure the Ani SDK is correctly installed."
#endif
ANI_EXPORT ani_status ANI_Constructor(ani_vm *vm, uint32_t *result) {
    ani_env *env;
    if (ANI_OK != vm->GetEnv(ANI_VERSION_1, &env)) {
        return ANI_ERROR;
    }
    ani_status status = ANI_OK;
    if (ANI_OK != string_io::ANIRegister(env)) {
        std::cerr << "Error from string_io::ANIRegister" << std::endl;
        status = ANI_ERROR;
    }
    if (ANI_OK != string_op::ANIRegister(env)) {
        std::cerr << "Error from string_op::ANIRegister" << std::endl;
        status = ANI_ERROR;
    }
    *result = ANI_VERSION_1;
    return status;
}
