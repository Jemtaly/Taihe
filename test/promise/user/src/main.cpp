...
auto make_future_value(T&& value, uint64_t ms) {
    return make_promise<T, E>([value = std::forward<T>(value),
                               ms](promise_view<T, E> p) {
        setTimeout(callback<void()>::from([p = promise_holder<T, E>(p), value,
                                           ms]() { p->resolve(value); }),
                   ms);
    });
}
...
auto make_future_error(E&& error, uint64_t ms) {
    return make_promise<T, E>([error = std::forward<E>(error),
                               ms](promise_view<T, E> p) {
        setTimeout(callback<void()>::from([p = promise_holder<T, E>(p), error,
                                           ms]() { p->reject(error); }),
                   ms);
    });
}
...
int main() {
    ...
    auto final_future_string =
        make_future_value<uint64_t, string>(42, 1000)
            ->chained_then([](int val) {
                std::cout << "Got value: " << val << std::endl;
                std::cout << "Please input in 3 sec:" << std::endl;
                return make_promise<string, string, IPromiseStringString>(
                    getInputWithTimeout, 3);
            })
            ->chained_then([](string_view sv) {
