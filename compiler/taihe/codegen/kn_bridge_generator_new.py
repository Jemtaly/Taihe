...
from taihe.codegen.abi_generator import IfaceDeclABIInfo
...
class IfaceTypeKnBridgeInfo(AbstractAnalysis[IfaceType], AbstractTypeKnBridgeInfo):
    def __init__(self, am: AnalysisManager, t: IfaceType):
        ...
        self.param_covert_func: str = "DerefStablePointer"
...
class KNBridgeCodeGenerator:
    ...
    def gen_need_decl(
        ...
        kn_bridge_pkg_target.write(
            ...
            f"}};\n"
            f"\n"
            f"struct KnClassDataBase {{\n"
            f"protected:\n"
            f"  void* m_handle;\n"
            f"public:\n"
            f"  KnClassDataBase(void* ptr): m_handle(ptr) {{}}\n"
            f"  virtual ~KnClassDataBase() {{\n"
    ...
    def gen_func_impl(
        ...
        for func in pkg.functions:
            if "init_class" not in func.attrs:
                ...
                # self.gen_makeclass(func, kn_bridge_pkg_target, str1, str2)
    ...
    def gen_makeclass(
        ...
        kn_bridge_pkg_target.write(
            f"{return_ty_name} {func.name}_author({params}) {{\n"
            f"  return make_holder<{return_ty_name}_impl, {return_ty_name}>({params_only_var});\n"
            f"}}\n"
            f"\n"
        )
    ...
    def _generate_init_function_body(
        ...
        kn_bridge_pkg_target.write(
            ...
            f"    {konan_proj_name}(result, {convert_params_str});\n"
            f"    return {return_ty_str};\n"
        )
    ...
    def gen_class(self, iface: IfaceDecl, kn_bridge_pkg_target: COutputBuffer):
        ...
        kn_bridge_pkg_target.write(
            f"struct {iface.name}_impl: KnClassDataBase {{\n" f"public:\n"
        )
        ...
        #     f"\n"
        # )
        # perantsList = IfaceDeclABIInfo.get(self.am, iface)
    ...
    def _gen_class_all_func_bind(
        ...
        perantsList = IfaceDeclABIInfo.get(self.am, iface).ancestor_dict
        if not perantsList:
            return
    ...
    def _gen_ctor(self, iface: IfaceDecl, kn_bridge_pkg_target: COutputBuffer):
        kn_bridge_pkg_target.write(
            f"  {iface.name}_impl({self.dict_params[iface.name]}):\n"
            f"    KnClassDataBase({self.dict_konan_func[iface.name]}_impl({self.dict_params_only_var[iface.name]})){{}};\n"
            f"\n"
        )
    ...
    def gen_func_macro(self, pkg: Package, kn_bridge_pkg_target: COutputBuffer):
        for func in pkg.functions:
            if "init_class" in func.attrs:
                kn_bridge_pkg_target.write(
